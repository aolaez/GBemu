...
static void proc_ld(cpu_context *ctx) {
    if (ctx->dest_is_mem) {
        if (is_16_bit(ctx->curr_instr->reg_2)) { // is 16 bit reg
            emu_cycles(1);
...
static void proc_ldh(cpu_context *ctx) {
    if (ctx->curr_instr->reg_1 == RT_A) {
        cpu_set_reg(ctx->curr_instr->reg_1, bus_read(0xFF00 | ctx->fetch_data));
    } else {
...
static void proc_add(cpu_context *ctx) {
    u32 val = cpu_read_reg(ctx->curr_instr->reg_1) + ctx->fetch_data;
    ...
    if (ctx->curr_instr->reg_1 == RT_SP) {
        val = cpu_read_reg(ctx->curr_instr->reg_1) + (char)ctx->fetch_data;
    }

    // flags
    int z = (val & 0xFF) == 0;
    int h = (cpu_read_reg(ctx->curr_instr->reg_1) & 0xF) + (ctx->fetch_data & 0xF) >= 0x10;
    int c = (int)(cpu_read_reg(ctx->curr_instr->reg_1) & 0xFF) + (int)(ctx->fetch_data & 0xFF) >= 0x100;

    if (is_16bit) {
        z = -1;
        h = (cpu_read_reg(ctx->curr_instr->reg_1) & 0xFFF) + (ctx->fetch_data & 0xFFF) >= 0x1000;
        u32 n = ((u32)cpu_read_reg(ctx->curr_instr->reg_1)) + ((u32)ctx->fetch_data);
        c = n >= 0x10000;
    }

    if (ctx->curr_instr->reg_1 == RT_SP) {
        z = 0;
        h = (cpu_read_reg(ctx->curr_instr->reg_1) & 0xF) + (ctx->fetch_data & 0xF) >= 0x10;
        c = (int)(cpu_read_reg(ctx->curr_instr->reg_1) & 0xFF) + (int)(ctx->fetch_data & 0xFF) >= 0x100;
    }
...
static void proc_sub(cpu_context *ctx) {
    u16 val = cpu_read_reg(ctx->curr_instr->reg_1) - ctx->fetch_data;

    int z = val == 0;
    int h = ((int)cpu_read_reg(ctx->curr_instr->reg_1) & 0xF) - ((int)ctx->fetch_data & 0xF) < 0;
    int c = ((int)cpu_read_reg(ctx->curr_instr->reg_1)) - ((int)ctx->fetch_data) < 0;
...
static void proc_sbc(cpu_context *ctx) {
    u16 val = ctx->fetch_data + CPU_FLAG_C;
