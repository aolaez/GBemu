...
static void proc_cp(cpu_context *ctx) {
    ...
    cpu_set_flags(ctx, n == 0, 1, 
        ((int)ctx->regs.a & 0x0F) - ((int)ctx->fetch_data & 0x0F) < 0, n < 0);
}
...
static void proc_ld(cpu_context *ctx) {
    ...
    if (ctx->curr_instr->mode == AM_HL_SPR) {
        ...
        cpu_set_reg(ctx->curr_instr->reg_1, 
            cpu_read_reg(ctx->curr_instr->reg_2) + (int8_t)ctx->fetch_data);
...
static void proc_add(cpu_context *ctx) {
    ...
    if (ctx->curr_instr->reg_1 == RT_SP) {
        val = cpu_read_reg(ctx->curr_instr->reg_1) + (int8_t)ctx->fetch_data;
    }
